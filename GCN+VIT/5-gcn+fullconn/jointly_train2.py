...
def writeEachEpoch(people, epoch, batchsize, lr, temperature, acc):
    ...
    with open(
            f'D:/demo/gcn+transformer/5-gcn+全连接/record/{DATASET}_All_log3.txt',
            'a') as f:
        f.writelines(log)
...
def updatelog(people, epoch, acc):
    ...
    with open(f'D:/demo/gcn+transformer/5-gcn+全连接/record/{DATASET}_UPDATE_LOG3.txt', 'a') as f:
        f.writelines(log)
...
def train(train_data, train_label, test_data, test_label, people):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 0:
            print()
            print(f'-----highest_acc {highest_acc:.4f} c` urrent_acc {current_acc:.4f}-----')
