...
def writeEachEpoch(people, epoch, batchsize, lr, temperature, acc):
    ...
    with open(
            f'D:/demo/gcn+transformer/5-gcn+densenet+VIT/record/{DATASET}_All_log1.txt',
            'a') as f:
        f.writelines(log)
...
def updatelog(people, epoch, acc):
    ...
    with open(f'D:/demo/gcn+transformer/5-gcn+densenet+VIT/record/{DATASET}_UPDATE_LOG1.txt', 'a') as f:
        f.writelines(log)
...
def train(train_data, train_label, test_data, test_label, people):
    ...
    # “sigmoid + BCE”或是“softmax + CE”
    net = JointlyTrainModel(5, 32, batch_size, testmode=False, HC=HC)
    net = net.to(device)
    criterion = nn.BCELoss().to(device)
    optimizer = Lamb(net.parameters(), lr=lr, weight_decay=weight_decay)
    ...
    for epoch in range(epochs):
        ...
        for ind, gdata in enumerate(gloader):
            ...
            x3 = net(gdata)
            y3 = gdata.y
            target_one_hot = F.one_hot(y3, 3).float()

            _, pred3 = torch.max(x3, dim=1)
            correct_pred3 += sum([1 for a,b in zip(pred3, y3) if a==b])

            softmax = nn.Softmax(dim=1)
            probabilities = softmax(x3)

            loss3 = criterion(probabilities, target_one_hot)
            loss = loss3
...
def runs(people):
    ...
    train_data, train_label = shuffle(train_data, train_label, random_state=1337)
    test_data, test_label = shuffle(test_data, test_label, random_state=1337)
