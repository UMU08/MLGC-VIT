...
def writeEachEpoch(people, epoch, batchsize, lr, temperature, acc):
    ...
    with open(
            f'D:/demo/gcn+transformer/5-gcn+attention+全连接/record/{DATASET}_All_log2.txt',
            'a') as f:
        f.writelines(log)
...
def updatelog(people, epoch, acc):
    ...
    with open(f'D:/demo/gcn+transformer/5-gcn+attention+全连接/record/{DATASET}_UPDATE_LOG2.txt', 'a') as f:
        f.writelines(log)
