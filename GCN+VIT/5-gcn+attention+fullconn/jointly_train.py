...
def writeEachEpoch(people, epoch, batchsize, lr, temperature, acc):
    ...
    with open(
            f'D:/demo/gcn+transformer/5-gcn+attention+全连接/record/{DATASET}_All_log1.txt',
            'a') as f:
        f.writelines(log)
...
def updatelog(people, epoch, acc):
    ...
    with open(f'D:/demo/gcn+transformer/5-gcn+attention+全连接/record/{DATASET}_UPDATE_LOG1.txt', 'a') as f:
        f.writelines(log)
...
def train(train_data, train_label, test_data, test_label, people):
    ...
    for epoch in range(epochs):
        ...
        writeEachEpoch(people, epoch, batch_size, lr*0.1, 0.25, current_acc)
