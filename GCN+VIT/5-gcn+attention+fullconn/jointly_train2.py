...
def writeEachEpoch(people, epoch, batchsize, lr, temperature, acc):
    ...
    with open(
            f'D:/demo/gcn+transformer/5-gcn+attention+全连接/record/{DATASET}_All_log3.txt',
            'a') as f:
        f.writelines(log)
...
def updatelog(people, epoch, acc):
    ...
    with open(f'D:/demo/gcn+transformer/5-gcn+attention+全连接/record/{DATASET}_UPDATE_LOG3.txt', 'a') as f:
        f.writelines(log)
