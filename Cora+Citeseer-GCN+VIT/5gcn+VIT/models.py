...
#         x6 = self.gc6(torch.cat([x, x1, x2, x3, x4, x5], 1), adj)
...
class GCN(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout):
        ...
        image_size = (10, 16)
        patch_size = (5, 4)
        self.vit_model = ViT(image_size=image_size, patch_size=patch_size, num_classes=nclass, dim=512, depth=6,
                             heads=16,
                             mlp_dim=512,
                             channels=1)
    ...
    def forward(self, x, adj):
        ...
        vit_x = vit_x.reshape(vit_x.shape[0], 1, 10, 16)
